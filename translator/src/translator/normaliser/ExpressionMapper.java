...
public class ExpressionMapper {
	...
	public translator.expression.Expression mapExpression(translator.conjureEssenceSpecification.Expression oldExpression) 
		...
		case EssenceGlobals.ATMOST:
			return mapAtmostConstraint(oldExpression.getAtmostConstraint());
	...
	/**
	 * Maps ATMOST and ATLEAST constraints (represented by translator.conjureEssenceSpecification.AtmostConstraint)
	 * to the advanced representation (translator.expression.Atmost).
	 * 
	 * @param oldAtmost
	 * @return
	 * @throws NormaliserException
	 */
	private Atmost mapAtmostConstraint(AtmostConstraint oldAtmost) 
		throws NormaliserException {
		
		// --- 1.map the array (1st argument) ------------------
		Expression arrayExpression = mapExpression(oldAtmost.getArray());
		if(!(arrayExpression instanceof Array))
			throw new NormaliserException("Illegal argument of ATMOST/ATLEAST constraint:"+oldAtmost
					+". Expected a decision variable of type array/matrix instead of '"+arrayExpression+" as first argument.");
		
		Array array = (Array) arrayExpression;
		
		return new Atmost(array,
				          oldAtmost.getOccurrences(),
				          oldAtmost.getValues(),
				          oldAtmost.isAtmost());
	}
	...
	protected translator.expression.Expression mapLexConstraint(LexExpression oldLexConstraint) 
		...
		Expression leftExpression = mapExpression(oldLexConstraint.getLeftExpression());
		Expression rightExpression = mapExpression(oldLexConstraint.getRightExpression());
		
		if(!(leftExpression instanceof Array)) 
			throw new NormaliserException("Incompatible type: can only impose lex constraint between two variable arrays, and not on:"+leftExpression);
		
		if(!(rightExpression instanceof Array)) 
			throw new NormaliserException("Incompatible type: can only impose lex constraint between two variable arrays, and not on:"+rightExpression);
		
		
		return new LexConstraint( (Array) leftExpression,
				                   mapOperator(oldLexConstraint.getLexOperator().getRestrictionMode()),
				                  (Array) rightExpression);
	...
	protected translator.expression.Expression mapAtomicExpression(AtomicExpression oldAtom) 
		...
		case EssenceGlobals.IDENTIFIER: 
			...
			// the identifier is a parameter
				...
				//System.out.println("Getting parameter domain "+domain+" for parameter:"+oldAtom);
	...
	protected translator.expression.Domain mapIntegerDomain(IntegerDomain intDomain) 
		...
		if(intDomain.getRangeList().length == 1) {
			...
			translator.expression.Expression lb = null;
			translator.expression.Expression ub = null;
			if(intDomain.getRangeList()[0].getLowerBound() == null) 
				lb = new ArithmeticAtomExpression(translator.expression.Expression.LOWER_BOUND);
			else lb = mapExpression(intDomain.getRangeList()[0].getLowerBound());
			
			if(intDomain.getRangeList()[0].getUpperBound() == null)
				ub = new ArithmeticAtomExpression(translator.expression.Expression.UPPER_BOUND);
			else ub = mapExpression(intDomain.getRangeList()[0].getUpperBound());
		...
		//  we have a sparse domain or mixed domain
			...
			while(i<rangeAtom.length) {
				...
				while(i<rangeAtom.length && rangeAtom[i].getRestrictionMode() == EssenceGlobals.RANGE_EXPR) {
					...
					//System.out.println("Iterating through sparse elements: "+i+". sparse element:"+sparseElement);
