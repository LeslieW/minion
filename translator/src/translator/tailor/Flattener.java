...
public class Flattener {
	...
	/** contains the offsets from zero for all decision variables' indices */
	HashMap<String, int[]> variableArrayOffsetsFromZero;
	...
	private Expression flattenAtmost(Atmost atmost) 
		...
		if((atmost.isAtmost() && this.targetSolver.supportsConstraint(Expression.ATMOST_CONSTRAINT)) ||
			...
			array = this.adaptArrayIndicesToSolver(array);
	...
	private Expression flattenLexConstraint(LexConstraint lexConstraint) 
		...
		leftArray = adaptArrayIndicesToSolver(leftArray);
		rightArray = adaptArrayIndicesToSolver(rightArray);
	...
	//System.out.println("NONCOMM left expression: "+leftExpression+" and right expression:"+rightExpression+" with op:"+expression.getOperator());
	leftExpression = flattenExpression(leftExpression);
					...
					if(rightExpression instanceof ArithmeticAtomExpression) {
						...
						if(this.normalisedModel.constantArrays.containsKey(arrayVar.getArrayNameOnly())) {
							...
							if(this.hasCommonSubExpression(arrayVar)) {
								...
								//System.out.println("Flattened to partwise element constraint: "+elementConstraint);
...
//						 if this array is a constant array -> just flatten it
		...
		if(!(lastConstraintLeft instanceof Reification && 
				((Reification) lastConstraintLeft).
				getReifiedVariable().toString().equals(leftExpression.toString()))) {
		...
		if(!(lastConstraintRight instanceof Reification && 
				((Reification) lastConstraintRight).
		...
		if(leftExpression.toString().equals(rightExpression.toString()) && 
				(expression.getOperator() == Expression.EQ || expression.getOperator() == Expression.IFF)) {
			/*Expression lastConstraint = this.constraintBuffer.get(this.constraintBuffer.size()-1);
			if(lastConstraint instanceof Reification) {
				Reification l = (Reification) lastConstraint;
				String reifVar = l.getReifiedVariable().getVariable().getVariableName();
				//System.out.println("Same constraint=, last reification: "+l);
				// if this is the last added aux var and it is the same as the left expression...
				if(reifVar.equals(this.AUXVARIABLE_NAME+(this.noAuxVariables-1)) && 
						leftExpression.toString().equals(reifVar)) {
	...
	private Expression flattenUnaryRelationalExpression(UnaryRelationalExpression expression) 
		...
		if(expression.getType() == Expression.NEGATION) {
			...
			if(expression.isGonnaBeFlattenedToVariable()) {
                return reifyConstraint(new AllDifferent(array));	
			}
			else {
				//System.out.println("Generated alldifferent: "+new AllDifferent(array));
	...
	private Expression flattenRelationalAtomExpression(RelationalAtomExpression atom) 
		...
		//System.out.println("Flattening the relational atom "+atom+" of type "+atom.getClass().getSimpleName());
					...
					// the solver does not support array indexing with something other than an integer
						...
						if(indices.length == 1) {
							...
							if(atom.isGonnaBeFlattenedToVariable()) {
							    return reifyConstraint(new ElementConstraint(array,//Variable,
															  index,
															  auxVariable));
							}
							else return new ElementConstraint(array, //Variable,
															  index,
															  auxVariable);
						}
				    ...
				    if(!arrayVariable.isIndexAdaptedToSolver() && 
						...
						*/
						for(int i=0; i<intIndices.length; i++) {
							...
							int offset = this.normalisedModel.getVariableOffsetAt(arrayVariable.getArrayNameOnly(),i);
							intIndices[i] -= offset;
						...
						// ---------- 2-dimensional arrays ------------------------------------------------
							...
							if(rowIndexExpression.getType() == Expression.INT) {
								...
								if(domain instanceof ConstantArrayDomain) {
									...
									int offset = this.normalisedModel.getVariableOffsetAt(arrayVariable.getArrayNameOnly(), 0);
									//System.out.println("COmputed row OFFSET:"+offset);
									row = row - offset;
...
//									 creating m[row,..]   (indexedArray)
									...
									//indexedArray = (IndexedArray) this.adaptArrayIndicesToSolver(indexedArray);	
									
									// prepare  index expression: index-offset
							...
							// ------------2.case: matrix[rowExpr:E, col:int]   ---> element(m[..,col],  rowExpr, aux) ---------
								...
								if(domain instanceof ConstantArrayDomain) {
									...
									int offset = this.normalisedModel.getVariableOffsetAt(arrayVariable.getArrayNameOnly(), 1);
									//System.out.println("COmputed col OFFSET:"+offset+" and column:"+col);
									col = col - offset;
									...
									// prepare  index expression: index-offset
									//System.out.println("Row index before offset stuff:"+rowIndexExpression);
									rowIndexExpression = new Sum(new Expression[] { rowIndexExpression},
	                      					 	                 new Expression[] {new ArithmeticAtomExpression(offset)});
									rowIndexExpression = rowIndexExpression.evaluate();
									//System.out.println("Row index after offset and eval:"+rowIndexExpression+" with type: "
									//		+rowIndexExpression.getClass().getSimpleName());
									if(!(rowIndexExpression instanceof AtomExpression) && 
											!this.targetSolver.supportsConstraintsNestedAsArgumentOf(Expression.ELEMENT_CONSTRAINT)) 
				...
				// else the indices are integer values
					...
					if(!arrayVariable.isIndexAdaptedToSolver() && 
						...
						//System.out.print("Adapting index at array variable "+atom);
					...
					if(indices.length == 1) {
						...
						// ------------2.case: matrix[rowExpr:E, col:int]   ---> element(m[..,col],  rowExpr, aux) ---------
							...
							if(domain instanceof ConstantArrayDomain) {
								...
								//System.out.println("Created partwise element constraint:"+new ElementConstraint(indexedArray,
				                 //          rowIndexExpression));
								// return element constraint
								return new ElementConstraint(indexedArray,
										                           rowIndexExpression);
	...
	private Expression insertConstantArraysInExpression(Expression expression) 
		...
		if(expression instanceof UnaryRelationalExpression)  {
			...
			if(expr.getOperator() == Expression.IF) {
				leftExpression = leftExpression.evaluate();
				if(leftExpression.getType() == Expression.BOOL) {
					return ((RelationalAtomExpression) leftExpression).getBool() ? 
							insertConstantArraysInExpression(expr.getRightArgument()) : 
			...
			if(atom.getType() == Expression.BOOL_ARRAY_VAR) {
				...
				if(arrayVariable.getType() == Expression.ARRAY_VARIABLE) {
					...
					if(indices!=null) {
						...
						//System.out.println("111111111111  Trying to find constant array variable match for :"+atom+" of type: "+atom.getClass().getSimpleName());
						
						for(int j=0; j<indices.length; j++) {
							//System.out.println("About to insert constant arrays into: "+indices[j]+" at index "+j);
							indices[j] = this.insertConstantArraysInExpression(indices[j]);
						//	System.out.println("Inserted constant array into index "+j+" with result: "+indices[j]);
						}
						//Expression e = new ArrayVariable(aVar.getArrayNameOnly(), 
						//			indices,
						//				aVar.getBaseDomain());
						//System.out.println("222222222222 After inserting expressions into atom:"+atom+" we get: "+e+
					//			" in which we will insert constant arrays again...");
						arrayVariable = new ArrayVariable(aVar.getArrayNameOnly(), 
												 										indices,
												 										aVar.getBaseDomain());
						expression = new RelationalAtomExpression(arrayVariable);
						//System.out.println("333333333333 After inserting expressions into atom:"+e+" we get: "+exp);
						//return exp;
					}
				...
				SimpleArrayVariable simpleVar = (SimpleArrayVariable) expression;
				//System.out.println("BEFORE XXX :"+simpleVar);
				Expression ee =insertConstantDomainIntoSimpleArrayVariable(simpleVar);
				//System.out.println("AFTER XXXX :"+ee+" of type:"+ee.getClass().getSimpleName());
				return ee;
	...
	throws TailorException,Exception {
		...
		if(this.normalisedModel.constantArrays.containsKey(simpleVar.getVariableName())) {
			...
			//System.out.println("Found a match for simple variable: "+simpleVar);
			...
			for(int i=indices.size()-1; i>=0; i--) {
				...
				//System.out.println("index "+i+" is "+index+" of type "+index.getClass().getSimpleName());
			...
			if(allIndicesAreSingleExpressions) {
				...
				for(int i=indexExpressions.length-1; i>=0;i--) {
					...
					indexExpressions[i] = this.insertConstantArraysInExpression(indexExpressions[i]);
					if(indexExpressions[i].getType() != Expression.INT)
						allints= false;
				}
				//Expression e = new ArrayVariable(variableName, 
				 //         indexExpressions, new InfiniteDomain());
				
				
				
				if(allints) {
					int[] intIndices = new int[indexExpressions.length];
					for(int i=intIndices.length-1; i>=0; i--) {
						intIndices[i] = ((ArithmeticAtomExpression) indexExpressions[i]).getConstant();
					}
					if(constantArray.getArrayDomain() != null) {
						if(!(constantArray.getArrayDomain() instanceof MatrixDomain))
							throw new Exception("Infeasible domain for constant array "+variableName+": "+constantArray.getArrayDomain()+
				...
				Expression e = new ArithmeticAtomExpression(new ArrayVariable(variableName, 
				          indexExpressions,
				          new InfiniteDomain()));
 ...
 */
	...
	private Array adaptArrayIndicesToSolver(Array array)
		...
		if(array instanceof IndexedArray) {
			...
			IndexedArray indexedArray = (IndexedArray) array;
			//System.out.println("About to adapt the array index of array: "+indexedArray);
			
			// extract the index domains
			BasicDomain[] indexDom = indexedArray.getIndexRanges();
			ConstantDomain[] indexDomain = new ConstantDomain[indexDom.length];
			for(int i=0; i<indexDomain.length; i++) {
				//System.out.println("Looking at index domain: "+indexDom[i]+" of type: "+(indexDom[i].getClass().getSimpleName()));
				if(indexDom[i] instanceof ConstantDomain)
					indexDomain[i] = (ConstantDomain) indexDom[i];
				// else we can't do much with it anyway if it;s not constant
				else return array;
			}
			
			String arrayName = indexedArray.getArrayName();
			for(int i=0; i<indexDomain.length; i++) {
				int offset = this.normalisedModel.getVariableOffsetAt(arrayName, i);
				
				// if we have to adapt to the solver
				if(offset != 0) {
					if(indexDomain[i] instanceof SingleIntRange) {
						int index = ((SingleIntRange) indexDomain[i]).getSingleRange();
						indexDomain[i] = new SingleIntRange(index-offset);
					}
					else if(indexDomain[i] instanceof BoundedIntRange) {
						int[] bounds = ((BoundedIntRange) indexDomain[i]).getRange();
						bounds[0] = bounds[0] - offset;
						bounds[1] = bounds[1] - offset;
						indexDomain[i] = new BoundedIntRange(bounds[0], bounds[1]);
					}
				}
				
			}
			
			indexedArray.setIndexRanges(indexDomain);
			//System.out.println("Adapted array index of array to: "+indexedArray);
			return indexedArray;
		}
