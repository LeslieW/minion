...
DynamicConstraint*
MakeBoolGreaterEqualSumConDynamic(const T1&, const T2&, const ConstraintBlob&, const BoolVarRef&)
{ 
  cerr << "Watched Sum only supported on sums of booleans to constant!" << endl; 
  exit(-1);
}

DynamicConstraint*
MakeBoolGreaterEqualSumConDynamic(const vector<BoolVarRef>& t1, const ConstantVar& sum, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{ 
...
template<typename T1, typename T2>     
DynamicConstraint*
MakeBoolLessEqualSumConDynamic(const T1&, const T2&, const ConstraintBlob&, const BoolVarRef&)
{ 
  cerr << "Watched Sum only supported on sums of booleans to constant!" << endl; 
  exit(-1);
}
...
DynamicConstraint*
MakeBoolLessEqualSumConDynamic(const vector<BoolVarRef>& t1, const ConstantVar& sum, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{ 
  runtime_val t2(sum.getAssignedValue());
...
struct BuildDynamicConstraint<2, 0>
{
  template<typename T1, typename T2>
  static 
  DynamicConstraint* build(const pair<pair<EmptyType, vector<T1>* >, vector<T2>* >& vars, const ConstraintBlob& b, int pos)
  { 
	BoolVarRef reifyVar;
	...
	switch(b.constraint.type)
	{
	  ...
	  default:
		D_ASSERT(0 && "unsupported constraint");
		exit(1);						
	}
  }
};
