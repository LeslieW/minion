...
int main (int argc, char * const argv[]) {
 ...
 // set_solution_function(&print_sol);
  ...
  for(int i=0;i<height;i++)
  {
    for(int j=0;j<width;j++)
      col[j]=vars[i][j];
    add_constraint(BoolLessEqualSumConDynamic(col,compiletime_val<widthsum>()));
    add_constraint(BoolGreaterEqualSumConDynamic(col,compiletime_val<widthsum>()));
  }
  ...
  for(int j=0;j<width;j++)
  {
    for(int i=0;i<height;i++)
      row[i]=vars[i][j];
    add_constraint(BoolLessEqualSumConDynamic(row,compiletime_val<heightsum>()));
    add_constraint(BoolGreaterEqualSumConDynamic(row,compiletime_val<heightsum>()));
  }
  ...
  for(int i=0;i<(height*(height-1))/2;i++)
  {
    for(int j=0;j<width;j++)
      col[j]=scalar[i][j];
    add_constraint(BoolLessEqualSumConDynamic(col,compiletime_val<scalarsum>()));
    add_constraint(BoolGreaterEqualSumConDynamic(col,compiletime_val<scalarsum>()));
  }
