...
public:
  ...
  void lock(int size, int min_domain_val, int max_domain_val)
  {
	...
	for(int i = 0; i < 4 + vars_domain_size; ++i)
  ...
  void allocateMem(char* mem_start)
  {
	...
	for(int i = 0; i < var_count_m * (4 + vars_domain_size); ++i)
	{
  ...
  {
#ifdef DYNAMICTRIGGERS
    dynamic_propogate(var_num, UpperBound);
#endif
	if(lock_second)
	{
  ...
  { 
#ifdef DYNAMICTRIGGERS
    dynamic_propogate(var_num, LowerBound);
#endif
	if(lock_second)
	{
  ...
  { 
#ifdef DYNAMICTRIGGERS
    dynamic_propogate(var_num, Assigned);
#endif
	if(lock_second)
	{
  ...
  { 
#ifdef DYNAMICTRIGGERS
    dynamic_propogate(var_num, DomainChanged);
#endif
	if(lock_second)
	{
  ...
  void addDynamicTrigger(int b, DynamicTrigger* t, TrigType type, int val)
  {
	...
	DynamicTrigger* old_list = t->next;
...
namespace TriggerSpace
{
  inline void finaliseTriggerLists()
  {
	...
	for(unsigned int i=0;i<trigger_lists.size();i++)
	{
	  int offset = trigger_lists[i]->memRequirement();
	  trigger_lists[i]->allocateMem(triggerlist_offset);
	  triggerlist_offset += offset;
	}
	D_ASSERT(triggerlist_offset - triggerlist_data == trigger_size);
  }
}
