...
struct ConstraintBlob
{
  ...
  /// List of tuples. Only used in Table Constraints.
...
/*  operator string()
  {
    ostringstream s;
    s << "ConType:" << constraint << "|";
    for(unsigned i = 0; i < vars.size(); ++i)
    s << string(vars[i]) << "`";
    if(reified)
      s << "Reified:" << string(reified_var);
    s << ".";
    return s.str();
  }*/
  ...
  struct VarContainer
{
 ...
 /* vector<Var> make_var_blob(vector<int> v)
  {
    vector<Var> vars(v.size());
    for(unsigned int i = 0; i < v.size(); ++i)
      vars[i] = get_var(v[i]);
    return VarArrayBlob(vars);
  }*/
  
  /*VarArrayBlob make_var_blob(int v)
  { return VarArrayBlob(get_var(v)); }*/
