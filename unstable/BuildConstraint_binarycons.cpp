...
Constraint*
MakeGreaterEqualSumCon(const vector<BoolVarRef>& t1, const ConstantVar& sum, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{ 
...
Constraint*
MakeLessEqualSumCon(const vector<BoolVarRef>& t1, const ConstantVar& sum, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{ 
  runtime_val t2(sum.getAssignedValue());
...
DEFCON3(ProductCon);
Constraint*
MakeProductCon(const BoolVarRef& t1,const BoolVarRef& t2,const BoolVarRef& t3, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{ FUNBODY3(AndCon); }
...
struct BuildConstraint<3, 0>
{
  ...
  static 
  Constraint* build(const pair<pair<EmptyType, vector<T1>* >, vector<T2>* >& vars, const ConstraintBlob& b, int pos)
  { 
	BoolVarRef reifyVar;
