...
struct BoolVarRef_internal
{
  ...
  {
    D_ASSERT(isAssigned());
    return (value_ptr() & shift_offset) ? 1 : 0;
  }
...
struct BooleanContainer
{
  ...
  void addLowerBoundTrigger(BoolVarRef_internal& b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_lower_trigger(b.var_num, t); }
  
  void addUpperBoundTrigger(BoolVarRef_internal& b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_upper_trigger(b.var_num, t); }
  
  void addAssignedTrigger(BoolVarRef_internal& b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_assign_trigger(b.var_num, t); }
  
  void addDomainChangedTrigger(BoolVarRef_internal& b, Trigger t)
