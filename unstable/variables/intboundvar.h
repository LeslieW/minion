...
struct BoundVarContainer {
  ...
  {
    D_ASSERT(0 && "Cannot Remove Value from domain of a bound var");
    exit(25);
  }
  ...
  void addLowerBoundTrigger(BoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_lower_trigger(b.var_num, t); }
  
  void addUpperBoundTrigger(BoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m);  trigger_list.add_upper_trigger(b.var_num, t);  }
  
  void addAssignedTrigger(BoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m);  trigger_list.add_assign_trigger(b.var_num, t);  }
  
  void addDomainChangedTrigger(BoundVarRef_internal b, Trigger t)
...
/*template<typename T>
inline BoundVarRef
BoundVarContainer<T>::get_new_var()
{
  D_ASSERT(!lock_m);
