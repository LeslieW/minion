...
struct SparseBoundVarContainer {
  ...
  /// find the largest possible lower bound below @new_lower_bound.
...
//  SparseBoundVarRef get_new_var();
  ...
  void addLowerBoundTrigger(SparseBoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_lower_trigger(b.var_num, t); }
  
  void addUpperBoundTrigger(SparseBoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_upper_trigger(b.var_num, t); }
  
  void addAssignedTrigger(SparseBoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_assign_trigger(b.var_num, t); }
  
  void addDomainChangedTrigger(SparseBoundVarRef_internal b, Trigger t)
  { D_ASSERT(lock_m); trigger_list.add_domain_trigger(b.var_num, t); }
...
/*template<typename T>
inline BoundVarRef
SparseBoundVarContainer<T>::get_new_var()
{
  D_ASSERT(!lock_m);
  initial_bounds.push_back(make_pair(var_min, var_max));
