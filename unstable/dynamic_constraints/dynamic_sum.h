...
/// This code has basically decayed beyound the point of working.. :(
  ...
  { 
    ...
    cout << "This almost certainly isn't going to work... sorry" << endl;
#endif
  }
  ...
  */
  ...
  virtual void full_propogate()
  {
	...
	}       // on exit index points to one more last array element looked at
  ...
  virtual void propogate(DynamicTrigger* dt)
  {
	...
	// and will need to loop round for watched lits	
        ...
        for(loop = 0 ; (not found_new_support) and loop < num_unwatched ; )
        {
          D_ASSERT(num_unwatched > 0);
  ...
  // Below is not changed in dt_sum, no idea if it works.
  
  /*  virtual bool check_unsat(int, DomainDelta)
  {
	int i = count.get() + 1;
	D_INFO(1,DI_DYSUMCON,to_string("Checking unsat, count",i));
	count.set(i);
	if(i > occ_count())
	  return true;
	else
