#ifdef CACHESUPPORT
#include "dynamic_cachesupport_element.h"
#else
...
struct ElementConstraintDynamic : public DynamicConstraint
{
  ...
  void find_new_support_for_result(int j)
  {
    ...
    int oldsupport = max(current_support[j + var_array.size()], indexvar.getMin());  // old support probably just removed
    ...
    current_support[j+var_array.size()] = support;
  }
  ...
  /*
   void deal_with_assigned_index()
   {
	 ...
	 int lower = resultvar.getMin(); // max(resultvar.getMin(),var_array[indexval].getMin());
	 int upper = resultvar.getMax(); // min(resultvar.getMax(),var_array[indexval].getMax());
	 
	 var_array[indexval].setMin(lower);
	 var_array[indexval].setMax(upper);
	 for(int i = lower; i <= upper; ++i)
	 {
	   if(!(resultvar.inDomain(i)))
		 var_array[indexval].removeFromDomain(i); 
	 }
   }
   */
