...
struct BUILDCON
{
  ...
  static 
  CONSTRAINT_TYPE* build(const ConData& partial_build, const ConstraintBlob& b, int pos);
};
...
build(const ConData& partial_build, const ConstraintBlob& b, int pos)
{
  ...
  if(same_type)
  {
	switch(type)
	{
	  ...
	  case VAR_SPARSEDISCRETE:	
		D_ASSERT(0 && "Sparse Discrete Variables current broken. Sorry"); exit(10);
  ...
  // This exit is here to stop a "no return in non-void function" warning. It should never be reached.
  D_ASSERT(0 && "This should never be reached.."); exit(2);
}  
...
{ cerr << "Cannot reify a watched constraint" << endl; exit(-1); } \
...
template<typename T1> \
 CONSTRAINT_TYPE* \
Make ## CON (const T1& t1,  const ConstraintBlob& b, const BoolVarRef& reifyVar) \
{ FUNBODY1(CON); }

#define DEFCON2(CON) \
template<typename T1, typename T2> \
CONSTRAINT_TYPE*\
Make ## CON (const T1& t1, const T2& t2, const ConstraintBlob& b, const BoolVarRef& reifyVar) \
{ \
  FUNBODY2(CON); \
}
...
CONSTRAINT_TYPE* \
Make ## CON (const T1& t1, const T2& t2, const T3& t3, const ConstraintBlob& b, const BoolVarRef& reifyVar) \
{ FUNBODY3(CON); }
