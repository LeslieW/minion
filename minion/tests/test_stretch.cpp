...
int main(void)
{
  ...
  LRangeVarRef a;
  a = rangevar_container.get_new_var(0,30);
  
  MultiplyVar<LRangeVarRef> mult_a(a,3);
  Controller::add_constraint(TestCon(mult_a));
  Controller::lock();
  Controller::propogate_queue();
  check_trigger_count(0,0,0,0);
  D_ASSERT(mult_a.getMin() == 0);
  D_ASSERT(mult_a.getMax() == 90);
  
  mult_a.setMin(2);
  D_ASSERT(mult_a.getMin() == 3);
  Controller::propogate_queue();
