...
namespace BuildCon
{
	...
	struct BuildConstraint<4, 0>
  {
	...
	static
	Constraint* build(const pair<pair<pair<EmptyType, vector<int>* >, vector<T1>* >, vector<T2>* >& vars, const ConstraintBlob& b, int)
  { 
	  BoolVarRef reifyVar;
	  ...
	  cout << "Serious internal error!?!?" << endl;
	  throw new int;
  }
	...
	static
	Constraint* build(const pair<pair<pair<EmptyType, vector<int>* >, vector<BoolVarRef>* >, vector<T2>* >& vars, const ConstraintBlob& b, int)
  { 
	  BoolVarRef reifyVar;
	  if(b.reified || b.implied_reified)
		reifyVar = boolean_container.get_var_num(b.reify_var.pos);
