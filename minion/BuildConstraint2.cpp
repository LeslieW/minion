...
get_AnyVarRef_from_Var(Var v)
{
  switch(v.type)
	...
	vector<char> build_val_and_var_order(ProbSpec::CSPInstance& instance)
  {
	  vector<char> final_val_order;
	  vector<AnyVarRef> final_var_order;
	  if(instance.var_order.size() != instance.val_order.size())
	  {
	    cout << "Variable order size is " << instance.var_order.size() << endl;
		cout << "Value order is " << instance.val_order.size() << endl;
		cout << "These must be the same. Sorry" << endl;
		exit(0);
	  }
	  ...
	  set_var_order(final_var_order);
	  return final_val_order;
  }	
  ...
  void build_variables(const ProbSpec::VarContainer& vars)
  {
	...
	{ cout << "Tell Chris to finish sparse discrete" << endl; D_ASSERT(0); exit(30); }
  }
