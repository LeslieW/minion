...
//
// Subversion Identity $Id: MinionInputReader.cpp 166 2006-05-09 09:40:23Z gentian $
//
...
void MinionInputReader::read(char* fn) {
  ...
  if(test_name != "MINION")
  {
    ...
    if (test_noparse) {exit(0);} 
    else {D_ASSERT(0); exit(4);}
  }
...
int main(int argc, char* argv[]) {
  ...
  if (argc == 1)
  {
	...
	<< "         [-timelimit]                Stop after given time" << endl
	<< endl
...
//  bool no_fail_in_setup = Controller::solve1(val_order);
...
//    Controller::solve_loop(var_order, val_order);
  ...
  // Solve!
  if(! reader.test_noparse) 
  { 
    Controller::solve(val_order);
  }
  else
  { 
    cerr << "Error! Should not have parsed successfully!" << endl;
    D_ASSERT(0); 
    exit(17);
  }
  ...
  if(Controller::test_mode)
  {
	...
	cout << "Solve Time: " << (end_time - setup_time) / (1.0 * CLOCKS_PER_SEC) << endl;
