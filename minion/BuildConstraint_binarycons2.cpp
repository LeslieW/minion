...
namespace BuildCon
{
  ...
  Constraint*
  MakeEqualCon(const T1& t1, const T2& t2, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{
	if(b.reified)
...
Constraint*
MakeEqualCon(const T1& t1, const ConstantVar& t2, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{
  if(b.reified)
	return ReifiedEqualCon(t1, t2, reifyVar);
  else if(b.implied_reified)
...
template<typename T1>
Constraint*
MakeNeqCon(const T1& t1, const ConstantVar& t2, const ConstraintBlob& b, const BoolVarRef& reifyVar)
{
  if(b.reified)
