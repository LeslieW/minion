...
enum debug_types
{ DI_SOLVER, DI_SUMCON, DI_BOOLCON, DI_ANDCON, DI_ARRAYAND, DI_QUEUE, DI_REIFY, DI_LEXCON, DI_TABLECON, DI_TEST,
  DI_DYSUMCON, DI_DYNAMICTRIG, DI_DYELEMENT, DI_INTCONTAINER, DI_GACELEMENT};
...
// importance: 0 = trivial, 1 = important, 2 = v. important.
  inline void D_INFO(int importance, debug_types x, std::string s)
  {
    ...
    switch(x)
    {
      ...
      default:
	std::cerr << "Missing debug case. Go add it to line 125(ish) in debug.h" << std::endl;
	D_ASSERT(0); exit(22);
    };
